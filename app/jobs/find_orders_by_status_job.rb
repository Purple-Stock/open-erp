class FindOrdersByStatusJob < ApplicationJob
    queue_as :default
    ...
    def process_order(order, tenant_id)
      ...
      begin
        ...
        if e.message.match?(/TOO_MANY_REQUESTS/) && retry_count < MAX_RETRIES
          retry_count += 1
          Rails.logger.warn("Retrying after TOO_MANY_REQUESTS error for order #{order.bling_order_id}. Attempt #{retry_count} of #{MAX_RETRIES}. #{e.message}")
          sleep(RETRY_DELAY)
          retry
        else
          Rails.logger.error("Failed to process order #{order.bling_order_id} after #{retry_count} retries: #{e.message}")
        end
      end
    end
